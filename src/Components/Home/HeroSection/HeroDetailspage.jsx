...
import HeroDetailsSkeleton from "../../Loading/HeroDetailsSkeleton";
...
const HeroDetailspage = () => {
    ...
    const { data: news, isLoading } = useQuery({
        queryKey: ["newsData"],
        queryFn: async () => {
            const res = await axios('/newdata.json'); 

            return res.data 
        }
    });

    if (isLoading) return <HeroDetailsSkeleton></HeroDetailsSkeleton>

    const singleNews = news?.find(s => s.id == id);

    if (!singleNews) return <div className="text-center py-20">News not found!</div>;
            ...
            <div className="max-w-4xl mx-auto px-4 py-6">
                ...
                <div className="flex flex-col md:flex-row gap-12 mt-10">
                    <div className="w-full md:w-3/4">
                        ...
                        {/* Dummy second paragraph for depth */}
                    ...
                    <div className="w-full md:w-1/4 border-l border-gray-100 pl-6 h-fit sticky top-24">
                        ...
                        <div className="space-y-6">
                            {news?.filter(n => n.id !== singleNews.id).slice(0, 3).map(item => (
                                <Link key={item.id} to={`/Details/${item.id}`} className="block group">
